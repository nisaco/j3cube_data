...
// ✅ FIX 1: Safe Date Formatter to prevent Blank Page
...
// ✅ FIX 2: Added useContext to prevent "theme is not defined" error
const TransactionCard = ({ tx }) => {
    const { theme } = useContext(ThemeContext); // <--- This line fixes the ReferenceError
...
const Button = ({ children, onClick, disabled = false, fullWidth = false, variant = 'primary', size = 'default' }) => {
  ...
  const variants = { primary: `hover:opacity-90`, outline: `border-2 ${theme.border} ${theme.textSub} hover:border-current`, dark: "bg-slate-800 text-white hover:bg-slate-900", danger: "bg-red-50 text-red-600 hover:bg-red-100", success: "bg-green-50 text-green-700 hover:bg-green-100 border border-green-200" };
...
const Dashboard = ({ user, transactions, setView, onTopUp }) => {
  ...
  <div className="space-y-6 animate-fade-in pb-20">
    <div className="rounded-3xl p-6 md:p-8 text-white shadow-xl relative overflow-hidden hover-scale" style={{ backgroundColor: theme.accentColor }}>
      <div className="relative z-10">
        <div className="flex justify-between items-start">
          <div><p className="opacity-80 font-medium mb-1 text-sm md:text-base">Wallet Balance</p><h1 className="text-3xl md:text-5xl font-bold tracking-tight">GHS {(user.walletBalance / 100).toFixed(2)}</h1></div>
        ...
        <div className="flex flex-col md:flex-row gap-3 mt-8">
          ...
          <button onClick={() => setView('history')} className="bg-black/20 text-white px-6 py-3 rounded-xl font-bold text-sm hover:bg-black/30 transition border border-white/20 backdrop-blur-md">View History</button>
        </div>
      </div>
    </div>

    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <button onClick={() => setView('purchase')} className={`${theme.cardBg} p-5 rounded-2xl border ${theme.border} shadow-sm flex items-center gap-4 text-left hover-scale group`}>
        <div className="w-14 h-14 rounded-2xl bg-opacity-10 flex items-center justify-center group-hover:scale-110 transition-transform" style={{ backgroundColor: `${theme.accentColor}20`, color: theme.accentColor }}><Wifi size={28} /></div>
      ...
      <button onClick={() => setView('console')} className={`${theme.cardBg} p-5 rounded-2xl border ${theme.border} shadow-sm flex items-center gap-4 text-left hover-scale group`}>
        ...
        <div><h3 className={`font-bold ${theme.textMain} text-lg`}>Developer API</h3><p className={`${theme.textSub} text-sm`}>Connect your App</p></div>
        <ChevronRight className={`ml-auto ${theme.textSub} opacity-50`} />
      </button>
    </div>
    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
          <h3 className={`text-lg font-bold ${theme.textMain}`}>Recent Transactions</h3>
      ...
      <div className={`hidden md:block ${theme.cardBg} rounded-2xl border ${theme.border} shadow-sm overflow-hidden`}>
        {filteredTransactions.length > 0 ? filteredTransactions.slice(0, 5).map((tx) => (
          <div key={tx._id} className={`p-4 border-b ${theme.border} flex items-center justify-between last:border-0 hover:bg-slate-50 transition`}>
            <div className="flex items-center gap-4">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${tx.status === 'data_sent' ? theme.successBadge : `${theme.inputBg} ${theme.textSub}`}`}>
                {tx.status === 'data_sent' ? <ArrowDownLeft size={18} /> : <ArrowUpRight size={18} />}
              </div>
              <div>
                  <p className={`font-bold ${theme.textMain} text-sm flex items-center`}>
                      {tx.dataPlan || 'Transaction'} 
                      <SourceBadge method={tx.paymentMethod} theme={theme} />
                  </p>
                  <p className={`text-xs ${theme.textSub}`}>{formatDateTime(tx.createdAt)}</p>
              </div>
            </div>
            <div className="text-right">
        ...
        )) : <div className={`p-8 text-center ${theme.textSub} text-sm`}>No transactions found</div>}
      </div>
    </div>
  </div>
  );
...
const AdminDashboard = ({ user, refreshUser }) => {
  const { theme } = useContext(ThemeContext);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        {(activeTab === 'users' || activeTab === 'orders') && (
            <div className={`relative flex-1 max-w-md`}>
                ...
                <input placeholder={activeTab === 'users' ? "Search users..." : "Search orders..."} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className={`w-full pl-10 pr-4 py-3 rounded-xl border ${theme.border} ${theme.inputBg} ${theme.textMain} outline-none focus:ring-2`} style={{ '--tw-ring-color': theme.accentColor }} />
            </div>
        )}
