  ...
  return (
    <div className="space-y-6 animate-in fade-in pb-20">
      ...
      {activeTab === 'overview' ? (
        ...
        <>
        {/* ✅ ADDED SEARCH BAR */}
        <div className="bg-white p-4 rounded-xl border border-slate-200 mb-4 shadow-sm flex items-center gap-3">
             <Search className="text-slate-400" size={20} />
             <input 
                type="text" 
                placeholder="Search orders by Username, Phone or Plan..." 
        ...
        <div className="space-y-3 md:hidden">
            {filteredOrders.length === 0 && <div className="text-center p-6 text-slate-500">No orders found matching "{searchTerm}"</div>}
        ...
        {/* DESKTOP TABLE VIEW */}
        <div className="hidden md:block bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center"><h3 className="font-bold text-slate-700">Recent Orders</h3><button onClick={fetchData} className="p-2 text-slate-500 hover:bg-white rounded-full"><RefreshCw size={16} /></button></div>
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                    ...
                    <tbody>
                        {filteredOrders.length === 0 && <tr><td colSpan="6" className="p-6 text-center text-slate-500">No orders found matching "{searchTerm}"</td></tr>}
                        {filteredOrders.map(order => (
...
export default function App() {
  ...
  const getFilteredHistory = () => {
       ...
       return transactions.filter(tx => {
           if(historyFilter === 'deposit') return tx.status === 'topup_successful';
           if(historyFilter === 'completed') return ['data_sent', 'success'].includes(tx.status);
           if(historyFilter === 'processing') return ['pending_review', 'processing'].includes(tx.status);
           if(historyFilter === 'failed') return ['data_failed', 'failed'].includes(tx.status);
           return true;
       });
  };
              ...
              {view === 'history' && (
                <div className="bg-white md:rounded-2xl md:p-6 md:shadow-sm">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 px-2 md:px-0">
                        <h2 className="font-bold text-xl md:text-2xl">Transaction History</h2>
                        ...
                        <div className="relative w-full md:w-auto">
                            ...
                            <select 
                                ...
                                className="w-full md:w-48 pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-sm text-slate-700 outline-none"
                            >
                    ...
                    {/* ✅ MOBILE CARDS */}
                    <div className="space-y-1 md:hidden">
                        {displayedHistory.length === 0 && <div className="p-8 text-center text-slate-400 text-sm bg-slate-50 rounded-xl">No transactions found for this filter.</div>}
                    ...
                    <div className="hidden md:block">
                        {displayedHistory.length === 0 && <div className="p-8 text-center text-slate-400 text-sm">No transactions found for this filter.</div>}
